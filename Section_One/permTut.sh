function permTut(){
    ...
    ponyNoClear applejack "After the first permissions block, the computer lists the owner of the file and then the group the file belongs to.  ${n2}The owner of the castle (the Princess!), can read, write, and execute (rwx).  Any of the poniess group can do the same!  Everyone not in the designated group is an "other" and is locked out (---) - the Princess is missing after all!"
    read -n 1 -r -p "Press any key to continue"
    clear
    ...
    ponyNoClear applejack "Let's see what we can find in this kitchen! Use ls -lah again to see who's got what food..."
    getInput 'ls -lah' 'Try again!'
    echo "total 14K"
    echo "drwxrwx--- 2 princess ponies 4.0K Aug  9 12:25 ."
    ...
    ponyNoClear applejack "Keep in mind anyone with (w)rite permissions to your files, including as a group member, can remove your files like we just did with the apples.${n2}We should make sure we can share the travelpack with the other ponies that are helping you, so we have to learn how to change the permissions of a file"
    read -n 1 -r -p "Press any key to continue"
    clear
    ...
    ponyNoClear applejack "You want to tell the computer that you want to change the group, what you want to change it to, and what you are changing.  So in this case, the command is as follows - give it a try!${n2}chgrp ponies other_weapons"
