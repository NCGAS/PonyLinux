...
owldir=${me}/owls
animaldir=${me}/animals
boulderdir=${me}/boulder
...
"A dark, musty hall lurks before you. Seemingly endless, featureless doors loom ahead through the gloom. It smells dank and moldy, like you are in the basement of an old building that occasionally floods. The floor has a fine layer of silt, causing your footsteps to scrape and shuffle as you walk. You feel doubt that you will ever find the key by groping randomly through these endless halls."
...
"Driving rain greets you as you open the door to the next room. You don't see anything through the sheet of water angrily pelting you from somewhere above. You run full-tilt to get to the other side, but you find yourself drenched to the bone. Your teeth chatter as you are soaked through-and-through, and the clammy dungeon walls offer neither respite nor shelter. You take 5 damage as your blood chills, and you run faster to try to keep warm. Just as you think to turn back, you see an awning ahead and dive for it. Under the awning, the sound of rain fades and you find yourself warm and dry once more. You wonder if this is the right way to be looking for this key. There are doors in front of you, painted in pastel colors and marked Door_ followed by three random seeming letters and numbers."
...
#######HERE#######
key="You find the key ground into the dust and rubble. You feel excited as you shake the key ring to get the dirt off, the jangling of metal ringing like bells of triumph. cd into the directory 'portal' in order to get back to the Princess' castle."
...
#Inserting animalfriends
                animaltest=$((RANDOM % $animalodds))
                #echo "rolling randomly $animaltest"
		if (( $animaltest == 0)); then
		    animalid=$((RANDOM % $numanimals))
		    cp ${animaldir}/${animals[$animalid]} .animal
		    chmod a+x .animal
                fi


#Inserting boulders
                bouldertest=$((RANDOM % $boulderodds))
                #echo "rolling randomly $bouldertest"
		if (( $bouldertest == 0)); then
		    cp ${boulderdir}/boulder .boulder
		    chmod a+x .boulder
		fi

#Inserting owls
                owltest=$((RANDOM % $owlodds))
                #echo "rolling randomly $owltest"
		if (( $owltest == 0)); then
		    owlid=$((RANDOM % $numowls))
	 	    cp ${owldir}/${owls[$owlid]} .owl
		    chmod a+x .owl
		fi
